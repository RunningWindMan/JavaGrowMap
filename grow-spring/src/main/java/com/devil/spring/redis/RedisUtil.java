...
public class RedisUtil {
    ...
    //自减Lua脚本
    private final String DECR_SCRIPT =
    ...
    public Cursor<String> scan(String pattern, int limit) {
        ...
        return (Cursor) redisTemplate.executeWithStickyConnection(
                (RedisCallback) redisConnection -> new ConvertingCursor<>(redisConnection.scan(options),
    ...
    public Map<String, String> hMGet(String key, Collection<String> fields) {
        List<Object> objectList = fields.stream().filter(Objects::nonNull).filter(field -> field.trim().length() != 0)
